 ...
 * e devolve ao client apenas as perguntas já filtradas e sem gabarito. O
 * módulo com o banco completo (lib/quiz-selection.ts → lib/questions-bank.ts)
 * nunca é importado por um componente client, então nunca entra no bundle.
 */
export async function fetchRestOfSession(seniority: SeniorityLevel): Promise<RestOfSession> {
  ...
  return {
    ...
    selfAssessmentQuestion: session.selfAssessmentQuestion,
  };
}
